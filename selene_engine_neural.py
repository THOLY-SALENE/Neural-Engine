"""
SELENE ENGINE v13 — FULL RESEARCH STACK (Fixed PPO + Enhanced Reward)

Fixes from v12.1:
- Correct GAE / advantage / returns shapes for multi-env
- Added tag handoff bonus + energy penalty
- Dataset export + eval mode
- Beautiful colored pulsing visualization
- Full CLI with seed control

Run:
  python selene_engine_research_v13.py --mode train --epochs 150 --seed 42
  python selene_engine_research_v13.py --mode animate
"""
...
def train(epochs=150, seed=42):
    print("=== SELENE v13 Training (Fixed PPO) ===")
    ...
    for ep in range(epochs):
        ...
        raw_adv = torch.cat(adv_list)
        adv = (raw_adv - raw_adv.mean()) / (raw_adv.std() + 1e-8)
        returns = raw_adv + torch.cat(val_buf[:-1])
    ...
    torch.save(policy.state_dict(), "selene_v13.pt")
    print("Training complete → selene_v13.pt saved\n")
...
def animate(seed=42):
    print("Launching beautiful simulation...")
    if Path("selene_v13.pt").exists():
        policy.load_state_dict(torch.load("selene_v13.pt", map_location=DEVICE))
    ...
    def update(f):
        ...
        ax.set_ylim(-6, 6)
        ax.set_zlim(-6, 6)
        ax.set_title("SELENE v13 — Neural Agents on Pulsing Torus", color="cyan", fontsize=12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SELENE ENGINE v13 — Full Research Stack")
